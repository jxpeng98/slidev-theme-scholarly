...
try {
  for (const template of expectedTemplates) {
    ...
    const pnpmWorkspacePath = path.join(target, 'pnpm-workspace.yaml')
    const slides = readText(slidesPath)
    const packageJson = readText(packagePath)
    const pnpmWorkspace = readText(pnpmWorkspacePath)
    ...
    expect(fs.existsSync(pnpmWorkspacePath), `${template.name} should create pnpm-workspace.yaml`)
    ...
    expectIncludes(`${template.name} pnpm-workspace.yaml`, pnpmWorkspace, 'citation-js: true')
