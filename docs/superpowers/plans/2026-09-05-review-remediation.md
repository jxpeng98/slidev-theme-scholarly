...
| 下一步 | 阶段 5C：真实 VS Code、高对比与读屏抽查 | 插入、新建已通过；继续空环境、标题错误持久反馈与读屏验收 |
...
- 完整视觉集成检查已通过，见 `phase-5-full-visual.log`；其中宿主测试编译后又修复了原生插入问题，已单独补跑到插件 33/33、lint、打包和原生验证。高对比抽查发现标题错误仅靠短时状态提示，持久字段反馈与真实读屏仍待补齐。
