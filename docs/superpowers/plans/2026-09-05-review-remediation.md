...
本次计划核对基于 `970c2f2` 及当前未提交改动。阶段状态以已有提交和执行记录为依据；本次计划更新未重新运行验收，不把历史通过结果视为当前所有场景已通过。
...
| 下一步 | 阶段 5B：同步最终截图与双语容量说明 | 34 个布局预览映射完整；Builder 截图反映当前界面；文档不承诺无限内容容量 |
| 然后 | 阶段 5C：真实 VS Code、高对比与读屏抽查 | 正确目标插入、新建与未保存场景、焦点与错误反馈有真实宿主证据 |
...
- 本地已执行 Node 20.20.2 和 24.14.0 的三份示例构建；这是 macOS 证据，不等于既有 Ubuntu、pnpm 10、全新安装的 CI。Node 20 日志中间有截断，最终记录应保留实际退出结果并注明日志限制。
- 原生 VS Code 安装已有成功记录，主要操作、高对比和真实读屏尚无完整通过证据。完整 `check:visual` 的最终一次集成结果也仍待补齐。
