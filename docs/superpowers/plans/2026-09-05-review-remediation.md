...
状态：执行中。阶段 1–4、派生资源同步及本地与远程集成检查已完成；原生宿主主流程和高对比抽查通过，实际读屏输出仍待验收。新增边界问题也纳入执行记录。

当前实现基于 `b5d4131`。最终总检查、局部浏览器回归、独立包检查及本地 Node 20/24 构建均已补跑；完整视觉检查与后续补验范围见执行记录。Ubuntu Node 20/24 构建和 Full Quality Gate 已通过，完整读屏输出仍未通过验收。
...
| 已完成 | 阶段 5D：最终集成检查、兼容性记录与分批提交 | 本地与 Ubuntu CI 均已有通过记录；计划仍须阶段 5C 的读屏证据才能关闭 |
...
| Ubuntu CI | `b5d4131` 的运行 [34158905555](https://github.com/jxpeng98/slidev-theme-scholarly/actions/runs/34158905555)：Node 20/24 三份示例构建与 Full Quality Gate 全部通过 | 通过，Ubuntu / pnpm 10 |
...
最终门槛仍为未全部就绪。本地与远程检查已通过，人工读屏结果仍待补齐；不创建发布标签，不发布包。
...
- 修复提交 `b5d4131` 已推送；远程复验 [34158905555](https://github.com/jxpeng98/slidev-theme-scholarly/actions/runs/34158905555) 全部通过：Node 20 构建 37 秒、Node 24 构建 32 秒、Full Quality Gate 50 秒。记录为 `remote-ci-34158905555.log` 与 `remote-ci-34158905555-watch.log`。既有 CI 未执行完整视觉导出；视觉验收仍引用前述本地实测证据。
