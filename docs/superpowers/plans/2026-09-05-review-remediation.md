...
- [ ] 追踪 Builder 首页合并、Slidev 全局标题解析和目录标题渲染，避免合法的 `title: false` 被当作全局字符串标题。修复共享路径，保留现有“隐藏目录标题”的意图和演示名称。
- [ ] 分别验证目录作为第一页及普通页面时，默认标题、自定义标题、隐藏标题三种情况；实际解析和浏览器加载通过，生成时不增加空白页、不产生重复 YAML 键。
- [ ] 将隐藏标题的首页纳入实际导出回归；如确需新增兼容参数，同步共享目录与双语说明，保留原有公开用法。
