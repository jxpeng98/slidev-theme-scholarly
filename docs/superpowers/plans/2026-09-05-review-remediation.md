...
状态：执行中。阶段 1–3 已修复并通过对应检查；继续编辑界面、文档响应式与集成验收。新增边界问题也纳入执行记录。

本次计划核对基于 `8ff73fe`。阶段状态以已有提交和执行记录为依据；本次更新未重新运行验收，不把历史通过结果视为当前所有场景已通过。

### 当前执行入口

| 顺序 | 下一项交付 | 验收后才能关闭的事项 |
|---|---|---|
| 已完成 | 四个提交修复检查、额外首页、布局草稿和操作校验 | R01、R02 主场景、R03、R08、R09；R11 随校验提前修复 |
| 下一步 | 修复实验矩阵与结果页的内容裁切 | R04：四组实验的设置、指标、数值、备注完整；浏览器与 PNG/PDF 分别验证 |
| 随后 | 接通示例引用，补齐目录首页标题边界 | R07：参考页与正文引用对应；R02 补充场景：隐藏目录标题且整份演示仍可播放、导出 |
| 然后 | 调整 Builder 窄窗口结构与键盘操作，约束文档 Hero | R05、R06、R10、R12；重验已修复的 R11 |
| 最后 | 同步预览与双语说明，完成宿主、兼容性和本地包验收 | 阶段 5 的全部必需项，不能以常规检查通过替代 |
...
- [x] 保留第一页面的正文、具名 slots、布局，以及演示的语言、主题、作者、参考文献和画幅配置；已覆盖封面重排与非封面首页。目录布局的布尔标题边界见阶段 3C，尚未验收。
...
- [ ] 在该入口补齐键盘焦点与窄窗口场景；将受影响演示/文档页面的可见性检查接入 `check:visual`。常规 `check` 保持无需启动浏览器；脚本管理本地服务并在失败时清理，不新增测试框架。
